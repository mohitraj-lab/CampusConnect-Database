...
**Security Protocols:** RSA Asymmetric Encryption & Diffie-Hellman Key Exchange
...
* **Insertion Anomaly:** You cannot add a new course until at least one student registers for it.
* **Deletion Anomaly:** If the only student enrolled in a course drops out, deleting their record completely wipes out the course details from the system.
* **Update Anomaly:** If a professor's email changes, you have to manually update thousands of enrollment rows, risking inconsistent data.
...
* **1NF:** Ensured every column contains atomic (indivisible) values and assigned a unique Primary Key to every table.
* **2NF:** Removed partial dependencies. Non-key attributes like `course_name` depend strictly on `course_id`, not on the composite key (`student_id`, `course_id`).
* **3NF:** Removed transitive dependencies. Department information and course metadata are separated so no non-key field depends on another non-key field.

---

### 2. Performance & Indexing Strategy

* **Foreign Key Indexes:** Indexed `student_id` and `course_id` in `enrollments` to speed up multi-table JOIN operations.
* **Composite Index:** Created a composite index on `(course_id, grade)` to optimize analytical queries filtering top-performing students by course.

---

### 3. Concurrency Control & Isolation Levels

* **Default Level (Repeatable Read):** MySQL InnoDB uses `REPEATABLE READ` with Next-Key Locking to prevent non-repeatable reads and phantom reads during concurrent seat bookings.
* **Race Condition Mitigation:** Handled concurrent course seat registration using explicit row locking (`SELECT ... FOR UPDATE`) inside a single database transaction.
